...
print(">>> [DEBUG] Running FSC_train.py from:", __file__)


def get_args_parser():
    parser = argparse.ArgumentParser('Finetuning on ViT', add_help=True)
    ...
    # Logging parameters
    parser.add_argument("--title", default="finetuning", type=str)
    parser.add_argument("--wandb", default="AdamW optimized fine-tuning", type=str)
...
def main(args):
    print("[DEBUG] args in FSC_train.py:", args)
    wandb_run = None
    try:
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            for data_iter_step, (samples, gt_density, _, pos_boxes, neg_boxes, pos, m_flag, im_names) in enumerate(
                ...
                pos_boxes = pos_boxes.to(device, non_blocking=True, dtype=torch.float32)
                neg_boxes = neg_boxes.to(device, non_blocking=True, dtype=torch.float32)
    ...
    # 更新 MAE 和 RMSE
                ...
                loss_scaler(total_loss, optimizer, clip_grad=0.8, parameters=model.parameters(), 
            ...
            for val_samples, val_gt_density, val_n_ppl, val_boxes,_, val_pos, _, val_im_names in \
                ...
                val_gt_density = val_gt_density.to(device, non_blocking=True, dtype=torch.float32)
                val_boxes = val_boxes.to(device, non_blocking=True, dtype=torch.float32)
