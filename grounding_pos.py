...
# robust GroundingDINO import
try:
    from groundingdino.util.inference import load_model, load_image, predict
except ModuleNotFoundError:
    import sys, os
    PROJECT_DIR = "/content/drive/MyDrive/Colab Notebooks/COMP 6970/VA-Count-Project_2"  # <-- adjust if needed
    sys.path.insert(0, os.path.join(PROJECT_DIR, "GroundingDINO"))
    from groundingdino.util.inference import load_model, load_image, predict
...
# --- START MODIFICATION ---
# Added if/else logic to check for the model and add print statements
model_weights_path = './data/out/classify/best_model.pth'

if os.path.exists(model_weights_path):
    print("--- Using TRAINED Classifier ---")
    print(f"Found 'best_model.pth', loading binary classifier...")
    ...
    print("Warning: 'best_model.pth' not found. Using DUMMY Classifier.")
    binary_classifier = None
# --- END MODIFICATION ---

# 判断 patch 是否有效
def is_valid_patch(patch, binary_classifier, preprocess, device, THRESH=0.50):
    # Dummy classifier path
    ...
    with torch.no_grad():
        ...
        # Handle BCE vs 2-class softmax outputs
    ...
    with open(text_file_path, 'r') as f:
        for line in f:
            ...
            for i, (box, logit) in enumerate(zip(patches, logits)):
                ...
                empty_crop   = (patch.size == (0, 0))
                too_wide     = (pw > w / 2)
                too_tall     = (ph > h / 2)
                too_narrow   = (pw < 5)
                too_short    = (ph < 5)

                # Call once so it doesn't run twice
                p_pos, ok = is_valid_patch(patch, binary_classifier, preprocess, device)

                reasons = []
                if empty_crop:  reasons.append("empty_crop")
                if not ok:      reasons.append(f"classifier_reject p={p_pos:.3f}")
                if too_wide:    reasons.append(f"too_wide pw={pw} > {w/2:.1f}")
                if too_tall:    reasons.append(f"too_tall ph={ph} > {h/2:.1f}")
                if too_narrow:  reasons.append(f"too_narrow pw={pw} < 5")
                if too_short:   reasons.append(f"too_short ph={ph} < 5")

                if reasons:
                    print(f"Skipping patch at box {box}: " + "; ".join(reasons))
                    continue
                top_patches.append((i, logit, patch))

            top_patches.sort(key=lambda x: x[1], reverse=True)
            top_3_indices = [patch[0] for patch in top_patches[:3]]
            
            
            ###Save images in annotated folder
            final_top_3 = top_patches[:3]
            base_name, _ = os.path.splitext(image_name)
            # Process the patches that were found
            for i, (patch_idx, logit, patch_to_save) in enumerate(final_top_3):
                # 1. Save the patch to disk
                output_filename = os.path.join(output_folder, f"{base_name}_pos_{i}.jpg")
                patch_to_save.save(output_filename, "JPEG")
            ...
            boxes_dict[image_name] = [(patches[idx].cpu().numpy() * np.array([w, h, w, h], dtype=np.float32)).tolist() for idx in top_3_indices]
